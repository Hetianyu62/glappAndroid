...
$(function() {
    ...
    initSelect("zip/area/", { "id": "" }, initFirstSel);
...
function initSelect(url, data, callback, el) {
    $("#app").publicAjax({
        ...
        //async:false,
        successFn: function(data) {
            callback(data.data, el);
        }
    })
}

function initFirstSel(data) {
    initSel(data, $("#p_sheng"));
    initSel(data, $("#p_n_sheng"))
 ...
 function IdentityCodeValid(val) {
			  var code=val
		            ...
		            var tip = "";
					 ...
					 var idval= $('#p_idcard').val()
					 if(idval != "" && idval != null){
						 if(!pass) //alert(tip)
						   top.layer.msg(tip,{ icon: 5, anim: 6 });
						  console.log(pass)
						 return pass;
					 }
